...
with PostgresSaver.from_conn_string(os.getenv('POSTGRES_URL')) as checkpointer:
    ...
    res = agent.invoke({'messages': [HumanMessage("Hi dude! Do you remember my name and where I live?")]}, config=config)
    res['messages'][-1].pretty_print()

    res = agent.invoke({'messages': [HumanMessage("What's the weather like?")]}, config=config)
    res['messages'][-1].pretty_print()